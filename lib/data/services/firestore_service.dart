...
class FirestoreService {
  ...
  /// Closes an attendance session and records absent status for enrolled students who did not scan.
  Future<void> closeAttendanceSession(String sessionId) async {
    ...
    try {
      await markAbsentForUnmarkedStudents(sessionId);
    } catch (e) {
      debugPrint('FirestoreService: Failed to mark absentees: $e');
    }
  }

  /// Automatically marks absent all enrolled students in the session's batch who did not scan attendance.
  Future<void> markAbsentForUnmarkedStudents(String sessionId) async {
    ...
    // Query enrolled students for this batch
    final studentsSnap = await _db
        .collection('students')
        .where('batchId', isEqualTo: session.batchId)
        .get();
    ...
    final recordedStudentIds = recordsSnap.docs
    ...
    for (final doc in studentsSnap.docs) {
      ...
      if (!recordedStudentIds.contains(studentUid)) {
        final recordId = '${sessionId}_$studentUid';
        final absentRecord = AttendanceRecordModel(
          id: recordId,
        ...
        batch.set(_db.collection('attendance_records').doc(recordId), absentRecord.toJson());
        count++;
        if (count >= 400) {
          await batch.commit();
          count = 0;
  ...
  /// Stream of all attendance sessions for a specific course and batch (active and closed).
  Stream<List<AttendanceSessionModel>> streamAllSessionsForBatch(
      String courseId, String batchId) {
    return _db
        .collection('sessions')
        .where('courseId', isEqualTo: courseId)
  ...
  // ══════════════════════════════════════════════════════════════════════
  // STUDENT PROFILE UPDATE REQUESTS (TEACHER APPROVAL WORKFLOW)
  // ══════════════════════════════════════════════════════════════════════

  /// Creates a profile update request from a student to be approved by faculty.
  Future<void> createProfileUpdateRequest(ProfileUpdateRequestModel request) async {
    await _db
    ...
    final notif = NotificationModel(
      ...
      body: 'Your profile changes have been submitted and are pending faculty approval.',
      type: NotificationType.system,
      isRead: false,
      createdAt: DateTime.now(),
    );
    await _db.collection('notifications').doc(notifId).set(notif.toJson());
  }

  /// Streams pending profile update requests for teachers/admins.
  Stream<List<ProfileUpdateRequestModel>> streamPendingProfileRequests() {
    return _db
        .collection('profile_update_requests')
        .where('status', isEqualTo: 'pending')
        .snapshots()
        .map((snap) => snap.docs
            .map((d) => ProfileUpdateRequestModel.fromJson(d.data()))
            .toList());
  }

  /// Streams the latest profile update request for a specific student.
  Stream<ProfileUpdateRequestModel?> streamStudentProfileRequest(String studentUid) {
    return _db
        .collection('profile_update_requests')
  ...
  /// Approves a profile update request and atomically validates and applies the changes.
  Future<void> approveProfileUpdateRequest({
    required String requestId,
    required String teacherUid,
    required String teacherName,
  }) async {
    final reqDoc = await _db.collection('profile_update_requests').doc(requestId).get();
    if (!reqDoc.exists || reqDoc.data() == null) return;
    final req = ProfileUpdateRequestModel.fromJson(reqDoc.data()!);

    final now = DateTime.now();
    final studentUid = req.studentUid;
    ...
    batch.update(_db.collection('profile_update_requests').doc(requestId), {
      ...
      'reviewedByTeacherId': teacherUid,
      'reviewedByTeacherName': teacherName,
    });

    // 2. Update student document
    final studentUpdates = <String, dynamic>{
      'courseId': req.requestedCourseId,
      'batchId': req.requestedBatchId,
      'section': req.requestedSection,
      'semester': req.requestedSemester,
      'user.name': req.requestedName,
    };
    if (req.requestedPhone != null) studentUpdates['user.phone'] = req.requestedPhone;
    if (req.requestedBio != null) studentUpdates['user.bio'] = req.requestedBio;
    batch.update(_db.collection('students').doc(studentUid), studentUpdates);

    // 3. Update base user document
    final userUpdates = <String, dynamic>{
      'name': req.requestedName,
      'updatedAt': now.toIso8601String(),
    };
    if (req.requestedPhone != null) userUpdates['phone'] = req.requestedPhone;
    if (req.requestedBio != null) userUpdates['bio'] = req.requestedBio;
    batch.update(_db.collection('users').doc(studentUid), userUpdates);
    ...
    final notif = NotificationModel(
      ...
      body: 'Your profile changes have been verified and approved by $teacherName.',
      type: NotificationType.system,
      isRead: false,
      createdAt: now,
    );
  ...
  /// Rejects a profile update request with an optional reason.
  Future<void> rejectProfileUpdateRequest({
    ...
    final now = DateTime.now();
    final studentUid = reqDoc.data()!['studentUid'] as String;
    ...
    batch.update(_db.collection('profile_update_requests').doc(requestId), {
      ...
      'reviewedByTeacherId': teacherUid,
      'reviewedByTeacherName': teacherName,
      'rejectionReason': reason ?? 'Profile edit was not approved by faculty.',
    });
    ...
    final notif = NotificationModel(
      ...
      body: reason != null && reason.isNotEmpty
          ? 'Your profile edit was declined by $teacherName: $reason'
          : 'Your profile update request was declined by $teacherName.',
      type: NotificationType.system,
      isRead: false,
      createdAt: now,
    );
